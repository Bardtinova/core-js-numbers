...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const multipkication = x1 * x2 + y1 * y2;
  const lenght = Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(multipkication / lenght);
}
...
function getSumOfDigits(num) {
  ...
  return arrayOfDigits.reduce((acc, cur) => acc + cur);
}
